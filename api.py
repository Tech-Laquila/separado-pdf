...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  #drop-area {
    ...
    border: 2px dashed #c5c8d0; border-radius: 16px;
...
const LABELS = { contrato: 'Contrato', procuracao: 'Procuração', declaracao: 'Declaração' };
const BADGES = { contrato: 'badge-contrato', procuracao: 'badge-procuracao', declaracao: 'badge-declaracao' };
...
async function processar(file) {
  ...
  try {
    ...
    if (!resp.ok) {
      status.innerHTML = '';
      results.innerHTML = '<div class="error">Erro: ' + (data.detail || 'Falha ao processar o PDF.') + '</div>';
      return;
    }
    ...
    data.documentos.forEach(doc => {
      ...
      card.innerHTML = `
        <div class="card-info">
          <div class="card-tipo">
            <span class="badge ${BADGES[doc.tipo]}">${LABELS[doc.tipo] || doc.tipo}</span>
          </div>
          <div class="card-meta">${doc.nome_arquivo} &nbsp;·&nbsp; ${doc.paginas} página(s)</div>
        </div>
        <a class="btn-download" href="${url}" download="${doc.nome_arquivo}">Baixar</a>
      `;
    ...
    status.innerHTML = '';
    results.innerHTML = '<div class="error">Erro de conexão. Verifique se o servidor está rodando.</div>';
  }
}
</script>
</body>
